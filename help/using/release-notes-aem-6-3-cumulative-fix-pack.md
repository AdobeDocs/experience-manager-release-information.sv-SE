...
|---|---|
| **Version** | 6,3 |
| **Frigör** | Kumulativt korrigeringspaket 6.3.3.8 den [Programvarudistribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq630/cumulativefixpack/aem-6.3.3-cfp-8.0.zip) |
| **Förutsättning** | [AEM 6.3 Service Pack 3 (6.3.3.0)](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions.html) |
...
Mer information om bestruket finpapper och andra typer av releaser finns i [Definitioner av underhållsreleaser för fordon.](https://experienceleague.adobe.com/docs/)
...
De viktigaste högdagrarna i **AEM Kumulativt korrigeringspaket** är:
...
* En komponents innehåll har ändrats i Author-instansen och sedan aktiverats visas inte i Publish-instansen förrän komponenten startas om **com.day.cq.personalization.impl.TargetedContentManagerImpl** (NPR-32489 och NPR-32232)
...
>[!NOTE]
>
>AEM Forms tilläggspaket hjälper till att anpassa formulärfunktionerna med AEM Service Pack och Cumulative Fix Packs. Därför är det viktigt att du installerar AEM Forms tilläggspaket när du har installerat AEM Service Pack, Cumulative Fix Pack eller Feature Pack.
...
* Dokumentsäkerhet: En skyddad PDF-fil kan inte öppnas offline med `DisableGlobalOfflineSynchronizationData` alternativ inställt på `True` (NPR-32080).

**Problem som har korrigerats i 6.3.0-0047**

* (Endast JEE) Allvarliga säkerhetsluckor (CVE-2021-44228 och CVE-2021-45046) rapporterade för Apache `Log4j2`.
...
* The `${extension}` variabeln renderas inte i författarinstansen vid arbetsflödesbearbetning (NPR-31694).
...
* Resurser replikeras till Publish-instansen från Author-instansen som körs på Dynamic Media - Scene7-distributionen, även efter att standardfilter har använts (NPR-30856).

### Sites {#sites-1}

* Det går inte att läsa in sidegenskaperna för en primär sida och ett NullPointerException returneras. Problemet löses när `cq:blueprin`t property (NPR-30901).
...
* Det går inte att öppna ett arkiv med ett procenttecken (%) i namnet med hjälp av gränssnittet Resurser när du packar upp en mapp. NPR-29989: Programfix för CQ-4270467
...
* Om du överför en Adobe Illustrator Artwork-fil med mer än 2 GB genereras ett Java™ heap space-fel. NPR-29265: Programfix för CQ-4226217
* Resurserna blir oanvändbara efter att DAM CQ Mime Type Service har tillämpat text för m3u8. NPR-29259: Programfix för CQ-4264052
...
* Felvarning genereras när komponenter läggs till på WCM-sidan när miniatyrbilder är aktiverade på instansen. NPR-29476: Programfix för CQ-4266197
...
* Data som tillhandahålls av användaren kan inte utelämnas vid utdata i `cq/replication/components/agent`-komponent, vilket resulterar i en lagrad XSS-säkerhetslucka (Cross-site scripting). NPR-29452: Programfix för CQ-4266263
...
* Värdena skiljer sig mellan samlingssidan och samlingssidan när du använder filter. NPR-28642: Programfix för CQ-4261405
...
* (Touch UI) Det går inte att uppdatera taggar i grupp till sidor som redan är taggade med olika taggar. NPR-28729: Programfix för CQ-4262922
* Om du öppnar variationen i kortvyn uppstår ett 500-fel. NPR-28611: Programfix för CQ-4263571
* Överrullning av en struktur som har flyttats i ett primärt leder till ett fel `cq:moveTarget`. NPR-28968: Programfix för CQ-4265280
...
* Svarsuppgift för delad kö öppnar ett flash-element på arbetsytan i HTML5. NPR-29161: Programfix för CQ-4266498
...
* Det går inte att ange mallprincip för kapslade parsys utan att ange inställningar för den överordnade komponenten. NPR-27987: Programfix för CQ-4246095
...
* DispatcherFlushRules kan bryta replikering i 6.3.3.1 NPR-28150: hotfix för CQ-4261401
...
* Ett undantag genereras med namnet `java.lang.IllegalArgumentException message:No enum constant com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE` som förhindrar att den kortvariga processen slutförs. NPR-28652
...
* Stöd har lagts till för fliken Regler och användningen av den i resursmappen i mappmetadataschemat.
...
* Förbättrad fråga så att den använder funktionen ExpiryNotification. NPR-26999: Programfix för CQ-4251188
...
* Hämtning via e-post för länkdelning returnerar en tom/skadad ZIP-fil. NPR-27997: Programfix för CQ-4259686
* Hybrid-videokodningen slutförs inte och ingen miniatyrbild skapas. NPR-27122: Programfix för CQ-4255080

#### Sites {#sites-6}

* Om den överordnade sidan stängs av tas cq:LiveRelationship-blandningstypen bort från den saknade sidan. NPR-26996: Programfix för CQ-4254113
...
* (Klassiskt användargränssnitt) Prestandaproblem med markeringen har ändrats till avlyssnare om det fanns flera listrutor. NPR-27115: Programfix för CQ-4237215
...
* Fokus/markör finns kvar i RTF-redigeraren även när du klickar på en alternativknapp i Safari. NPR-27144: Programfix för CQ-4249635
...
* The `cq:actions` beaktas inte för en riktad komponent. NPR-27616: Programfix för CQ-4257497
...
* AEM Forms Container-komponenten blir osynlig när fel formulär konfigureras på webbplatssidan och kryssrutan &quot;formulär täcker hela sidans bredd&quot; är markerad. NPR-25972: Programfix för CQ-4239287, CQ-4249133
...
* Added config servlet for the edit config for the RTE in dialog box mode.
...
* Om du lägger till specialtecken rullas dialogrutan Textredigeraren uppåt. NPR-26435: Programfix för CQ-4249869
...
* Om du vill skapa en variant som Live Copy måste användaren uppdatera sidan så att den återspeglar. NPR-26663: Programfix för CQ-4254328
...
* Förhandsvisningen av PDF och HTML är synlig och fungerar i samma fönster när en redigerbar modul redigeras, sparas och sedan PDF-förhandsvisning öppnas/stängs. NPR-26770: Programfix för CQ-4253217
...
* Om du anger en sökmall från administratörsgränssnittet och sedan använder samma för att söka efter uppgiften på arbetsytan i AEM Forms genereras ett undantag. Programfix för CQ-4255649
* Spårningsinformation för processerna under programmen med parentessymbolen i namnet visas inte på arbetsytan i HTML. Programfix för CQ-4242101
* Om du sparar ändringar på inställningsskärmen på arbetsytan i HTML genereras ett undantag. Programfix för CQ-4241485
...
* Sökning efter sparade sökningar i smarta samlingar returnerar inte alla resultat. NPR-23243: Programfix för CQ-4240355
...
* Det går inte att anteckna i MP4-videoresurs för en rendering i Dynamic Media S7 (DMS7). NPR-2046: Programfix för CQ-4215912
...
* Åtgärda omsorterbara alternativ i listvyn. NPR-23065: Programfix för CQ-4239321
...
* Problem med communitykomponenter som aktiveras via Dispatcher och sling dynamic. NPR-23586: Programfix för CQ-4242360, CQ-4241522
...
* När du rullar ut sidan visas inte de egenskaper för På/av-tid som ärvts från mallsidan. NPR-23873: Programfix för CQ-4243431
...
* (FDM) (WebService) Stöd för WSDL-tilläggets konstruktion i WSDL Parser. NPR-23640, NPR:23236: Programfix för 4205821
...
* Uppdaterade felix-http-paket för att aktivera sessionsattribut.
...
* HTTP 504: Gateway-timeoutfel vid försök att dela samlingar med hjälp av delningslänk. NPR-21928: Programfix för CQ-4234507
...
* RTE-helskärm I dialogrutan Touch UI döljer RTE-plugin-program. NPR-2034: Programfix för CQ-4235457
* (Touchgränssnitt) RTF-redigeraren tar bort alla andra attribut än id från taggen. NPR-2044: Programfix för CQ-4234133
...
* (Klassiskt användargränssnitt) RTF-redigerarutdata html lägger till &quot;list-position-style: inside;&quot; som infogat format i &lt;ul> -tagg. NPR-22145: Programfix för CRTE-114
...
* När komponenten öppnas i infogat läge visas inte plugin-program som lästs in tidigare vid andra gången. NPR-22591: Programfix för CQ-4236850
...
* (Hantera publikation) Om du vill gå till nästa kalendermånad måste du klicka flera gånger. NPR-23508: Programfix för CQ-4242732
...
* PersonalizationDataSource genererar ett Null-pekarundantag på grund av att jcr:content-noden saknas. NPR-22850: Programfix för CQ-4222122
...
* Anpassade funktioner för regelredigeraren lägger till en extra funktion. Efter funktionsanrop misslyckas valideringen även om den anpassade funktionen returnerar true. NPR-22481: Programfix för CQ-4235499
...
* (HTML Workspace) Skärmstorleken påverkas inte av rullningslister. NPR-23288
* (HTML Workspace) Processstartpunkter sorteras inte i alfanumerisk ordning. Programfix NPR-22841 för CQ-4238944
* (HTML Workspace) Förbered data aktiveras när formuläret stängs på arbetsytan. NPR-21127: Programfix för CQ-4224574
* (Arbetsytan HTML) När du anropar en process som kräver anteckningar med långa beskrivningar fungerar inte expanderingsknappen Anteckningar. Programfix för CQ-4241488
...
* Ett undantag uppstod vid försök att digitalt signera/certifiera ett PDF-dokument med HSM. NPR-21154: Programfix för CQ-4226978
...
* ProductAssetsUploader lämnar de temporära filerna för Java™-cache i Java™ TEMP-mappen. NPR-20256: Programfix för CQ-4221801
...
* Oväntad tagg i slutet av det inklistrade innehållet i innehållsfragmentet. NPR-2013: Programfix för CQ-4224014
...
* Beteendeändring i visning av titlar på bildkort för bild med dc: title-egenskap inställd på String [] (multifield).
...
* Publicerade JSON API för att använda innehållsfragment.
...
* Publicerade JSON API för att använda innehållsfragment. NPR-19500
...
* Renderingsproblem med AEM artikelkonsol som användning av bilder i full storlek gör det trögt. NPR-1908
...
* När du använder webbläsaren Google Chrome kan användare inte läsa in den fullständiga listan med sidor/resurser med kolumnvyn i TouchUI. NPR-19566: Programfix för CQ-4214248
...
* Korrigera fältet&quot;aktiverat&quot; i användargränssnittet för Brand Portal-molntjänstkonfigurationen. Programfix för CQ-4211101
...
* UTF-8-tecken som utdata från SOAP-tjänsten kopieras inte korrekt till adaptiva Forms-fält för andra språk än engelska. NPR-20238: NPR-20103
...
* Flerradigt textfält (TextArea) behåller inte nya radtecken eller radbrytningar i text när data har skickats i arbetsytan HTML. NPR-20085
...
>[!NOTE]
>
>Installera LiveCyclets inbäddningspaket som anges i [AEM Forms Releases](aem-forms-releases.md) artikel som åtgärdar problemet.
...
* När duplicerade resurser överförs används alternativet (ta bort/behåll) för alla resurser även när det inte är markerat i en identifierad dialogruta. CQ-4201673
...
* Ostängd resurslösare i `com.day.cq.wcm.core.impl.VersionManagerImpl`. NPR-17789: CFP for CQ-4211152
...
* Extra marginal ovanför och under en punktlista visas i textredigeraren men inte i HTML och PDF. NPR-18126
...
* PDF Generator kan inte konvertera bilder som är större än 100 MB till PDF-dokument. Ref# CQ-4208628
...
>
>Dokumentation för alternativet Validera finns på [https://docs.adobe.com/content/docs/en/aem/6-3/administer/content/package-manager.html#Package%20Validator](https://docs.adobe.com/content/docs/en/aem/6-3/administer/content/package-manager.html#Package%20Validator)

* AEM 6.3.3.0 är en förutsättning för den gemensamma fiskeripolitiken. Besök [Uppgradera dokumentation](https://docs.adobe.com/docs/en/aem/6-3/deploy/upgrade.html) för detaljerade anvisningar om uppgradering av en AEM installation till AEM 6.3.
...
1. Klicka på [Programvarudistribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq640/cumulativefixpack/aem-6.4.8-cfp-2.0.zip) för att ladda ned paketet.
...
* Använd [HTTP-API från Package Manager](https://helpx.adobe.com/experience-manager/6-3/sites/administering/using/package-manager.html) - försäkra dig om att du använder `cmd=install&recursive=true` - så det kapslade paketet installeras.

### Validera installation {#validate-installation}

1. Sidan Produktinformation ( `/system/console/ productinfo`) ska nu visa den uppdaterade versionssträngen &quot;Adobe Experience Manager, version 6.3.3.8&quot; under Installerade produkter.
1. Alla OSGI-paket är antingen AKTIVA eller FRAGMENT i OSGI-konsolen (Använd webbkonsolen: `/system/console/bundles`).

>[!NOTE]
>
>När paketet har installerats visas ett informationsmeddelande i felloggarna som anger att innehållspaketet har installerats, till exempel&quot;Innehållspaket&quot; **AEM-6.3.3-Cumulative-fix-pack-7** har installerats.&quot;
...
1. Installera Forms tilläggspaket enligt beskrivningen i [Installera AEM formulärtilläggspaket](https://helpx.adobe.com/experience-manager/6-3/forms/using/installing-configuring-aem-forms-osgi.html).
...
1. Kör Designer 6.2.0 för att installera uppdateringen&lt;language>_Cumulative_QF.msp-fil.
...
Med AEM Cumulative Fix Pack 6.3.0.2 och senare versioner är OSGi-konfigurationen som används för integrering av Search &amp; Promote **Proxykonfiguration för Apache HTTP-komponenter**. Proxykonfigurationen från Day Commons HTTP Client 3.1 används inte längre.

## Kända fel {#known-issues}

* Följande fel och varningar kan förekomma under installation av AEM CFP 6.3.3.x och kan ignoreras utan problem:

   * &#42;VARNING&#42; [OsgiInstallerImpl] org.apache.jackrabbit.vault.packaging.impl.InstallHookProcessorImpl Hook /META-INF/vault/hooks/cloudservices-wfchangeinstallkrok-0.0.2-jar-with-Berodencies.jar utlöste ett körningsfel.
   * &#42;FEL&#42; [OsgiInstallerImpl] com.adobe.cq.social.cq-social-jcr-provider [com.adobe.cq.social.provider.jcr.impl.SpiSocialJcrResourceProviderImpl(2174)] Tidsgränsen överskreds i väntan på att reg.ändringen skulle slutföras utan registrering. CQ-4209974.
   ...
   * &#42;FEL&#42; [FelixDispatchQueue] com.day.cq.dam.cq-dam-core FrameworkEvent ERROR (org.osgi.framework.ServiceException: Service factory returnerade null. (komponent: com.day.cq.dam.handler.standard.ps.PostScriptHandler)
...
>[!NOTE]
>
>Om du påverkas av något av dessa två problem kan du kontakta [AEM kundtjänst](https://helpx.adobe.com/se/contact/enterprise-support.ec.html).
...
>[!NOTE]
>
>Den här konfigurationsinställningen gäller för CFP 6.3.3.2 och senare. Den instruerar dig att uppdatera egenskaperna för startdelegering i `sling` .properties-fil.
...
* Öppna adobe-livecycle-cq-author.ear/cq.war/WEB-INF/web.xml för redigering:
   ...
   * uppdatera **sling.bootdelegering.ibm** param-name-värde med:
...
Egenskapen styr det maximala djupet för nodunderträdet under sidan ` /jcr:content` nod till vilken noderna i databasen ska användas för att hämta sidegenskaperna. Alla noder som finns under det angivna djupet i den här egenskapen ignoreras.

Standardvärdet är 1. Värdet kan åsidosättas genom att filen constants.js ( ersätts`/libs/cq/ui/widgets/source/constants.js`) som avkommenterar egenskapen CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL och tilldelar den det värde som krävs (det största djupet under sidans /jcr:innehåll som sidegenskapens data lagras i).
...
Uber Jar för 6.3.3.8 finns på [Adobe Public Maven-arkivet](https://repo.adobe.com/nexus/content/groups/public/com/adobe/aem/uber-jar/6.3.3.8/).

Om du vill använda Uber Jar i ett Maven-projekt kan du läsa artikeln, [Hur du använder Uber jar](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/ht-projects-maven.html) och inkludera följande beroende i projektens POM:
...
|----|-----|-----|-----|
| Resurser och integrering med Adobe Creative Cloud | [AEM till Creative Cloud-mappdelning](https://helpx.adobe.com/experience-manager/6-3/sites/administering/using/creative-cloud.html) infördes i AEM 6.2 som ett sätt att ge kreativa användare tillgång till resurser från AEM. Adobe Asset Link är en ny funktion som lanserats i Creative Cloud som ger en mycket bättre användarupplevelse och kraftfullare åtkomst till AEM direkt inifrån Photoshop, InDesign och Illustrator.<br /> Adobe kommer inte att förbättra mappdelningsfunktionen ytterligare. Funktionen ingår i AEM, men kunderna rekommenderas att använda den. | Adobe Asset Link eller Desktop App. Mer information finns på [Integrering med AEM Creative Cloud](https://helpx.adobe.com/experience-manager/6-3/assets/using/aem-cc-integration-best-practices.html) artikel. | AEM 6.3.3.x |
